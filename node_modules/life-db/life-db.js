module.exports = {
	...
	insert : function(databaseName, pageName, data) {
		var presentObject = this;
		if(!presentObject.checkIfDatabaseExists(databaseName)) {
	...
	checkIfDatabaseExists : function(databaseName) {
		var fs = require('fs');
		return fs.exists(this.rootFolderName+"/"+databaseName);
	},
	checkIfPageExists : function(databaseName, pageName) {
	...
	getInitialPageContent : function(databaseName, pageName) {
		var initialContent = {};
		initialContent.databaseName = databaseName;
		initialContent.pageName = pageName;
		initialContent.ceatedOn = new Date().getTime();
