module.exports = {
	...
	*/
	...
	find : function(databaseName, pageName, query, callBack) {
		...
		presentObject.checkIfPageExists(databaseName, pageName, function(flag) {
			if(flag) {
				...
				var pagePath = presentObject.rootFolderName + "/" + databaseName + "/" + pageName + presentObject.pageFileExtension;
				presentObject.getPageContent(pagePath, function(data){
					var pageData = presentObject.getReadableContent(data);
					if(query.trim()=="" || query.trim()=="*") {
						callBack(pageData);
					} else {
						var orSeparatedQueryArray = presentObject.splitQueryByOR(query);
						var data = presentObject.applyOrSeparatedQueryOnData(pageData, orSeparatedQueryArray);
						callBack(data);		
					}
				});				
						
			} else {
				console.log("ERROR : page doesnot exists");
			}
		});
		
	},

	applyOrSeparatedQueryOnData : function(pageData, orSeparatedQueryArray) {
		var presentObject = this;
		var tempData = JSON.parse(pageData)["data"];
		var finalData = [];
		if(tempData.length == 0) {
			return tempData;
		} else {
			for(var dataIndex in tempData) {
				var dataChunk = tempData[dataIndex];
				var flag = 0;
				for(var orSeparatedQueryIndex in orSeparatedQueryArray) {
					var andSeparatedQueryArray = presentObject.splitQueryByAND(orSeparatedQueryArray[index]);
					var andResult = true;
					for(var andSeparatedQueryIndex in andSeparatedQueryArray) {
						var query = andSeparatedQueryArray[andSeparatedQueryIndex];
						var result = presentObject.applyQueryToData(data, query);
						if(!result) {
							andResult = false;
							break;
						}
			...
			// for(var index in orSeparatedQueryArray) {
			// 	var andSeparatedQueryArray = presentObject.splitQueryByAND(orSeparatedQueryArray[index]);
			// 	for(var index2 in andSeparatedQueryArray) {
	...
	applyQueryToData : function(data, query) {
		
	},

	splitQueryByAND : function(query) {
		return query.split(this.operator.AND);
	},
