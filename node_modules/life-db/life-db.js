	...
	insert : function(databaseName, pageName, data) {
		...
		if(!presentObject.checkIfDatabaseExists(databaseName)) {// if database doesn't exists
			...
			if(!presentObject.checkIfPageExists(databaseName, pageName)) {
				presentObject.createPage(databaseName, pageName, presentObject.initiateInsert(databaseName, pageName, data));
			} else {
				presentObject.initiateInsert(databaseName, pageName, data);
			}
		}
	},
	initiateInsert : function(databaseName, pageName, data) {
		var presentObject = this;
		var databasePath = presentObject.rootFolderName+"/"+databaseName;
		var pagePath = databasePath + "/" + pageName + ".db";
		var pageContent = presentObject.getPageContent(databaseName, pageName);
	},
